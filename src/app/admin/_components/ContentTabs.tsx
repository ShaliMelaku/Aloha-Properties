...
export function MarketingTab({ 
  ...
  const [broadcastMode, setBroadcastMode] = useState<'individual' | 'imported'>('individual');
  const [individualEmails, setIndividualEmails] = useState("");
  const [importedBroadcastEmails, setImportedBroadcastEmails] = useState<string[]>([]);
  ...
  const handleBatchImport = async () => {
    ...
    try {
      ...
      // ── Store imported emails for direct broadcasting ──────────────────────
      const emails = importPreview.map(l => l.email).filter((e): e is string => !!e);
      setImportedBroadcastEmails(emails);

      onNotify('success', `Imported ${success} leads.${failed > 0 ? ` ${failed} failed.` : ''} You can now broadcast directly to them.`);
        ...
        {marketingSubTab === 'outreach' ? (
          <motion.div key="outreach" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            <div className="bg-[var(--card)] rounded-3xl md:rounded-[2.5rem] border border-[var(--border)] p-6 md:p-10 shadow-sm space-y-8 md:space-y-10 relative overflow-hidden group">
              ...
              <div className="relative z-10 grid grid-cols-2 gap-2 p-1.5 bg-slate-500/5 rounded-2xl">
                <button
                  ...
                  className={`py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                    broadcastMode === 'individual' ? 'bg-brand-blue text-white shadow-lg shadow-brand-blue/20' : 'opacity-40 hover:opacity-100'
                  }`}
                >
                  <Mail size={12} /> Specific Emails
                </button>
                <button
                  onClick={() => setBroadcastMode('imported')}
                  className={`py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                    broadcastMode === 'imported' ? 'bg-brand-blue text-white shadow-lg shadow-brand-blue/20' : 'opacity-40 hover:opacity-100'
                  }`}
                >
                  <Database size={12} /> Imported CSV Emails
                </button>
              </div>

              <div className="space-y-6 relative z-10">

                {/* ── Individual / Imported Emails mode ─────────────────── */}
                {broadcastMode === 'individual' && (
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <label htmlFor="individual-emails" className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-4">Recipient Emails (comma-separated)</label>
                      <textarea
                        ...
                        className="w-full px-6 py-4 rounded-2xl bg-[var(--background)] font-bold text-xs border border-[var(--border)] focus:border-brand-blue outline-none resize-none"
                      />
                    </div>
                  </div>
                )}

                {/* ── Imported CSV Emails mode ────────────────────────────── */}
                {broadcastMode === 'imported' && (
                  <div className="space-y-2 p-6 bg-[var(--background)] rounded-2xl border border-[var(--border)] text-center">
                    <Database className="mx-auto text-brand-blue opacity-50 mb-3" size={28} />
                    <p className="text-xs font-black uppercase tracking-widest">
                      {importedBroadcastEmails.length} Imported Emails Ready
                    </p>
                    {importedBroadcastEmails.length === 0 && (
                      <p className="text-[10px] opacity-40 uppercase font-bold tracking-widest mt-3 px-4">
                        You have not imported any emails yet. Use the &quot;Import List (CSV)&quot; section below to load leads first.
                      </p>
                    )}
                  </div>
                )}
                 ...
                 <div className="space-y-2">
                    ...
                    <textarea rows={8} placeholder="Type your message here..." value={body} onChange={e => setBody(e.target.value)} className="w-full px-6 py-5 rounded-2xl bg-[var(--background)] text-sm border border-[var(--border)] focus:border-brand-blue outline-none resize-none" />
                 </div>
                 <button onClick={async () => {
                    ...
                    if (broadcastMode === 'individual') {
                      emailsToSend = individualEmails.split(',').map(s => s.trim()).filter(s => s);
                      if (emailsToSend.length === 0) { onNotify('error', 'Please enter at least one recipient email.'); return; }
                    } else if (broadcastMode === 'imported') {
                      emailsToSend = importedBroadcastEmails;
                      if (emailsToSend.length === 0) { onNotify('error', 'No imported emails available. Please use the Import List option below first.'); return; }
                    }
                    ...
                    try {
                      ...
                      if (data.success) onNotify('success', `Broadcast sent to ${data.sent} contact${data.sent !== 1 ? 's' : ''}.`);
