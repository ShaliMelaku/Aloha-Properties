...
import { 
  ...
  Box, Users, FileText, CreditCard, Percent, Clock, Check
} from "lucide-react";
...
import { saveUnitType, saveUnit, saveProgress, deleteUnit, deleteUnitType } from "@/lib/admin-actions";
...
interface ProductsTabProps {
  ...
  setConfirmDelete: (v: { type: 'property' | 'post' | 'lead' | 'unit' | 'unitType' | 'milestone', id: string, name: string } | null) => void;
             ...
             <div className="p-8 flex-1 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                   <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest opacity-40">
                      ...
                      <div className="flex items-center gap-1 cursor-pointer hover:text-brand-blue" onClick={() => { 
                        setEditingProperty(prop);
                        setActivePropertyId(prop.id); 
                        const firstProg = (prop.progress?.[0] || {}) as Partial<PropertyProgress>;
                        setEditingProgress({
                          ...firstProg,
                          label: firstProg.label || firstProg.status_text || '',
                          percentage: firstProg.percentage ?? firstProg.percent ?? 0
                        }); 
                      }}>
                        <TrendingUp size={12} /> {(prop.progress?.[0]?.percentage ?? prop.progress?.[0]?.percent ?? 0)}% Complete
                      </div>
                   </div>
                    ...
                    <div className="space-y-6 bg-[var(--background)] p-8 rounded-3xl border border-[var(--border)]">
                       <div className="flex justify-between items-center">
                          <div className="flex items-center gap-3">
                             ...
                             <h4 className="text-xs font-black uppercase tracking-widest opacity-60">Financing Options</h4>
                          </div>
                          <button
                             type="button"
                             onClick={() => {
                                const current = editingProperty?.financing_available ?? newProp.financing_available ?? false;
                                if (editingProperty) setEditingProperty({...editingProperty, financing_available: !current});
                                else setNewProp({...newProp, financing_available: !current});
                             }}
                             className={`relative w-14 h-7 rounded-full transition-all duration-300 ${
                                (editingProperty?.financing_available ?? newProp.financing_available) ? 'bg-brand-blue' : 'bg-slate-500/20'
                             }`}
                             title="Toggle financing availability"
                          >
                             <span className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow transition-all duration-300 ${
                                (editingProperty?.financing_available ?? newProp.financing_available) ? 'left-8' : 'left-1'
                             }`} />
                          </button>
                       </div>

                       {(editingProperty?.financing_available ?? newProp.financing_available) && (
                         <div className="space-y-6">
                           <div className="space-y-2">
                              <label className="text-[9px] font-black uppercase tracking-widest opacity-40 ml-1">Financing Types (select all that apply)</label>
                              <div className="flex flex-wrap gap-2">
                                {(['Mortgage', 'Installment', 'Lease-to-Own', 'Developer Finance', 'Bank Loan', 'Islamic Finance'] as const).map(type => {
                                  const currentTypes = editingProperty?.financing_types ?? newProp.financing_types ?? [];
                                  const isActive = currentTypes.includes(type);
                                  return (
                                    <button key={type} type="button"
                                      onClick={() => {
                                        const updated = isActive ? currentTypes.filter(t => t !== type) : [...currentTypes, type];
                                        if (editingProperty) setEditingProperty({...editingProperty, financing_types: updated});
                                        else setNewProp({...newProp, financing_types: updated});
                                      }}
                                      className={`flex items-center gap-1.5 px-4 py-2 rounded-xl border text-[9px] font-black uppercase tracking-widest transition-all ${
                                        isActive ? 'border-brand-blue bg-brand-blue/10 text-brand-blue' : 'border-[var(--border)] opacity-50 hover:opacity-100'
                                      }`}
                                    >
                                      {isActive && <Check size={10} />}{type}
                                    </button>
                                  );
                                })}
                              </div>
                           ...
                           <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                              <div className="space-y-2">
                                <label className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest opacity-40 ml-1"><Percent size={10}/>Min Down Pay. %</label>
                                <input type="number" min="0" max="100" step="0.5" placeholder="20"
                                  value={editingProperty?.financing_down_payment_pct ?? newProp.financing_down_payment_pct ?? ''}
                                  onChange={e => { const v = parseFloat(e.target.value); if (editingProperty) setEditingProperty({...editingProperty, financing_down_payment_pct: v}); else setNewProp({...newProp, financing_down_payment_pct: v}); }}
                                  className="w-full px-4 py-3 bg-[var(--card)] rounded-xl text-xs font-bold border border-[var(--border)] focus:border-brand-blue outline-none" />
                              </div>
                              <div className="space-y-2">
                                <label className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest opacity-40 ml-1"><Clock size={10}/>Duration (Months)</label>
                                <input type="number" min="1" step="1" placeholder="120"
                                  value={editingProperty?.financing_duration_months ?? newProp.financing_duration_months ?? ''}
                                  onChange={e => { const v = parseInt(e.target.value); if (editingProperty) setEditingProperty({...editingProperty, financing_duration_months: v}); else setNewProp({...newProp, financing_duration_months: v}); }}
                                  className="w-full px-4 py-3 bg-[var(--card)] rounded-xl text-xs font-bold border border-[var(--border)] focus:border-brand-blue outline-none" />
                                {(() => { const m = editingProperty?.financing_duration_months ?? newProp.financing_duration_months; return m ? <p className="text-[8px] font-black opacity-30 ml-1">{(m/12).toFixed(1)} yrs</p> : null; })()}
                              </div>
                              <div className="space-y-2">
                                <label className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest opacity-40 ml-1"><Percent size={10}/>Interest Rate %/yr</label>
                                <input type="number" min="0" step="0.1" placeholder="0"
                                  value={editingProperty?.financing_interest_rate ?? newProp.financing_interest_rate ?? ''}
                                  onChange={e => { const v = parseFloat(e.target.value); if (editingProperty) setEditingProperty({...editingProperty, financing_interest_rate: v}); else setNewProp({...newProp, financing_interest_rate: v}); }}
                                  className="w-full px-4 py-3 bg-[var(--card)] rounded-xl text-xs font-bold border border-[var(--border)] focus:border-brand-blue outline-none" />
                              </div>
                              <div className="space-y-2">
                                <label className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest opacity-40 ml-1"><CreditCard size={10}/>Provider</label>
                                <input type="text" placeholder="e.g. CBE, Awash Bank"
                                  value={editingProperty?.financing_provider ?? newProp.financing_provider ?? ''}
                                  onChange={e => { if (editingProperty) setEditingProperty({...editingProperty, financing_provider: e.target.value}); else setNewProp({...newProp, financing_provider: e.target.value}); }}
                                  className="w-full px-4 py-3 bg-[var(--card)] rounded-xl text-xs font-bold border border-[var(--border)] focus:border-brand-blue outline-none" />
                              </div>
                           </div>
                           <div className="space-y-2">
                              <label className="text-[9px] font-black uppercase tracking-widest opacity-40 ml-1">Eligibility Conditions &amp; Notes</label>
                              <textarea rows={3} placeholder="e.g. Requires proof of income, minimum 2 years employment..."
                                value={editingProperty?.financing_notes ?? newProp.financing_notes ?? ''}
                                onChange={e => { if (editingProperty) setEditingProperty({...editingProperty, financing_notes: e.target.value}); else setNewProp({...newProp, financing_notes: e.target.value}); }}
                                className="w-full px-4 py-3 bg-[var(--card)] rounded-xl text-xs border border-[var(--border)] focus:border-brand-blue outline-none resize-none" />
                           </div>
                         </div>
                       )}
                    </div>
                     ...
                     {editingProperty ? (
                        <div className="pt-8 border-t border-[var(--border)] mt-8">
                           <div className="flex justify-between items-center mb-6">
                              ...
                              {!editingProgress && (
                                <button onClick={() => { setActivePropertyId(editingProperty.id); setEditingProgress({ label: '', percentage: 0 }); }} className="flex items-center gap-2 text-[10px] font-black uppercase text-brand-blue hover:opacity-70 transition-all"><Plus size={12}/> Add Milestone</button>
                              )}
                           </div>
                           <div className="space-y-3">
                              {editingProgress && !editingProgress.id && (
                                <div className="p-4 bg-brand-blue/5 rounded-xl border border-brand-blue/20 space-y-4">
                                   <p className="text-[8px] font-black uppercase text-brand-blue tracking-widest">New Milestone</p>
                                   <div className="flex gap-2">
                                      <input 
                                        type="text" 
                                        placeholder="e.g. Foundation Works"
                                        value={editingProgress.label || ''} 
                                        onChange={e => setEditingProgress({...editingProgress, label: e.target.value})}
                                        className="flex-1 bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-[10px] font-bold outline-none focus:border-brand-blue"
                                      />
                                      <input 
                                        type="number" 
                                        placeholder="%"
                                        ...
                                        className="w-16 bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-[10px] font-bold outline-none focus:border-brand-blue"
                                      />
                                   </div>
                                   <div className="flex justify-end gap-2">
                                      <button onClick={() => setEditingProgress(null)} className="px-3 py-1.5 rounded-lg text-[8px] font-black uppercase opacity-40 hover:opacity-100 transition-all">Cancel</button>
                                      <button onClick={handleSaveProgress} className="px-3 py-1.5 bg-brand-blue text-white rounded-lg text-[8px] font-black uppercase shadow-lg shadow-brand-blue/20">Create Milestone</button>
                                   </div>
                                </div>
                              )}
                           </div>
                           <div className="space-y-3">
                              {(properties.find(prop => prop.id === (editingProperty as Property).id)?.progress || []).map((p: PropertyProgress) => (
                                 <div key={p.id} className="p-4 bg-[var(--background)] rounded-xl border border-[var(--border)] group transition-all">
                                    {editingProgress?.id === p.id ? (
                                      <div className="space-y-4">
                                         <div className="flex gap-2">
                                            <input 
                                              type="text" 
                                         ...
                                         <div className="flex justify-end gap-2">
                                            <button onClick={() => setEditingProgress(null)} className="px-3 py-1.5 rounded-lg text-[8px] font-black uppercase opacity-40 hover:opacity-100 transition-all">Cancel</button>
                                            <button onClick={handleSaveProgress} className="px-3 py-1.5 bg-brand-blue text-white rounded-lg text-[8px] font-black uppercase shadow-lg shadow-brand-blue/20">Save</button>
                                         </div>
                                      </div>
                                    ) : (
                                      <div className="flex items-center justify-between">
                                         ...
                                         <div className="flex gap-2">
                                            <button onClick={() => { 
                                               ...
                                               setEditingProgress({
                                                 ...p,
                                                 label: p.label || p.status_text || '',
                                                 percentage: p.percentage ?? p.percent ?? 0
                                               }); 
                                            }} className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-blue/10 text-brand-blue rounded-lg hover:bg-brand-blue hover:text-white transition-all text-[8px] font-black uppercase" title="Edit Milestone">
                                              <Settings2 size={10}/> Edit
                                            </button>
                                            <button onClick={() => setConfirmDelete({ type: 'milestone', id: p.id, name: p.label || p.status_text || 'Milestone' })} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500 hover:text-white transition-all text-[8px] font-black uppercase" title="Delete Milestone">
                  ...
                  <div className="grid grid-cols-1 gap-6">
                     ...
                     {editingUnitType && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="bg-brand-blue/5 border border-brand-blue/20 rounded-[2.5rem] p-10 space-y-8">
                           ...
                           {/* ── Per-Type Financing Overrides ────────────────── */}
                           <div className="space-y-4 p-6 bg-[var(--background)] rounded-2xl border border-[var(--border)]">
                              <div className="flex justify-between items-center">
                                 <div className="flex items-center gap-2">
                                    <CreditCard size={13} className="text-brand-blue opacity-60" />
                                    <p className="text-[9px] font-black uppercase tracking-widest opacity-40">Financing Override for this Type</p>
                                 </div>
                                 <button type="button"
                                   onClick={() => {
                                     const cur = editingUnitType.financing_available;
                                     const next = cur === null || cur === undefined ? true : cur === true ? false : null;
                                     setEditingUnitType({...editingUnitType, financing_available: next});
                                   }}
                                   className={`px-3 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest border transition-all ${
                                     editingUnitType.financing_available === true ? 'border-brand-blue bg-brand-blue/10 text-brand-blue' :
                                     editingUnitType.financing_available === false ? 'border-red-500/30 bg-red-500/5 text-red-400' :
                                     'border-[var(--border)] opacity-40'
                                   }`}
                                 >
                                   {editingUnitType.financing_available === true ? '\u2713 Enabled' : editingUnitType.financing_available === false ? '\u2715 Disabled' : 'Inherit from Property'}
                                 </button>
                              </div>
                              {editingUnitType.financing_available === true && (
                                <div className="grid grid-cols-3 gap-3">
                                  <div className="space-y-1">
                                    <label className="text-[8px] font-black uppercase opacity-30 ml-1">Down Pay. %</label>
                                    <input type="number" min="0" max="100" step="0.5" placeholder="Inherit"
                                      value={editingUnitType.financing_down_payment_pct ?? ''}
                                      onChange={e => setEditingUnitType({...editingUnitType, financing_down_payment_pct: parseFloat(e.target.value)})}
                                      className="w-full px-3 py-2.5 bg-[var(--card)] rounded-xl text-[10px] font-bold border border-[var(--border)] focus:border-brand-blue outline-none" />
                                  </div>
                                  <div className="space-y-1">
                                    <label className="text-[8px] font-black uppercase opacity-30 ml-1">Duration (Months)</label>
                                    <input type="number" min="1" step="1" placeholder="Inherit"
                                      value={editingUnitType.financing_duration_months ?? ''}
                                      onChange={e => setEditingUnitType({...editingUnitType, financing_duration_months: parseInt(e.target.value)})}
                                      className="w-full px-3 py-2.5 bg-[var(--card)] rounded-xl text-[10px] font-bold border border-[var(--border)] focus:border-brand-blue outline-none" />
                                  </div>
                                  <div className="space-y-1">
                                    <label className="text-[8px] font-black uppercase opacity-30 ml-1">Rate % / yr</label>
                                    <input type="number" min="0" step="0.1" placeholder="Inherit"
                                      value={editingUnitType.financing_interest_rate ?? ''}
                                      onChange={e => setEditingUnitType({...editingUnitType, financing_interest_rate: parseFloat(e.target.value)})}
                                      className="w-full px-3 py-2.5 bg-[var(--card)] rounded-xl text-[10px] font-bold border border-[var(--border)] focus:border-brand-blue outline-none" />
                                  </div>
                                </div>
                              )}
