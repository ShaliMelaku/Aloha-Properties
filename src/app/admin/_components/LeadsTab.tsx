              ...
              <tr key={lead.id} className="hover:bg-brand-blue/5 transition-all group">
                ...
                <td className="px-8 py-6 text-right">
                  <div className="flex justify-end gap-2">
                    <button onClick={() => setViewingLead(lead)} title="View & Edit CRM" aria-label="View Node" className="w-9 h-9 rounded-lg border border-[var(--border)] flex items-center justify-center opacity-40 hover:opacity-100 hover:border-brand-blue hover:text-brand-blue transition-all"><Eye size={14} /></button>
                    <button onClick={() => setConfirmDelete({ type: 'lead', id: lead.id!, name: lead.name })} title="Delete" aria-label="Purge Node" className="w-9 h-9 rounded-lg border border-[var(--border)] flex items-center justify-center text-red-400 opacity-0 group-hover:opacity-100 hover:bg-red-500 hover:text-white transition-all"><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        {viewingLead && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-[var(--card)] rounded-[3rem] border border-[var(--border)] w-full max-w-2xl p-10 space-y-8 relative overflow-y-auto max-h-[90vh]">
              ...
              <div className="space-y-1">
                <h4 className="text-3xl font-heading font-black tracking-tighter uppercase">Prospect <span className="opacity-30 italic">Profile.</span></h4>
