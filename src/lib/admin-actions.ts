...
const ALLOWED_PROPERTY_FIELDS = [
  ...
  'financing_available', 'financing_types', 'financing_down_payment_pct',
  'financing_duration_months', 'financing_interest_rate', 'financing_provider',
  'financing_notes'
];
...
export async function saveProgress(prog: ProgressInput) {
  ...
  const payload = {
    ...
    percent: Number(rest.percentage ?? rest.percent ?? 0),
    status_text: rest.label || rest.status_text || 'New Phase',
    status: (Number(rest.percentage ?? rest.percent ?? 0) >= 100) ? 'delivered' : 'under-construction'
  };
  ...
  if (id && id.length > 5) { // Robust ID check
    const { data, error } = await supabaseClient
      ...
      .single();
    if (error) throw error;
    await logActivity('Progress Updated', 'progress', id, payload.status_text);
    return data;
  } else {
    const { data, error } = await supabaseClient
      .from('property_progress')
      ...
      .single();
    if (error) throw error;
    await logActivity('Progress Created', 'progress', data.id, payload.status_text);
    return data;
  }
}
