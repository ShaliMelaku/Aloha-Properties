...
export function SocialPulse() {
  ...
  const getContextualInfo = () => {
    ...
    const progressVal = (activePulse.progress?.[0]?.percentage ?? activePulse.progress?.[0]?.percent ?? 0);
    const isConstruction = progressVal > 0 && progressVal < 100;
    if (isConstruction) {
       return {
         icon: TrendingUp,
         text: `Phase ${activePulse.progress![0].label || activePulse.progress![0].status_text} active. Demand at 94%.`,
