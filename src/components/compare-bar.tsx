                      ...
                      <div key={prop.id} className={`rounded-3xl border overflow-hidden ${isTop ? 'border-brand-blue/40 shadow-xl shadow-brand-blue/10' : 'border-[var(--border)]'}`}>
                        ...
                        <div className="p-4 space-y-3 bg-[var(--card)]">
                          {[
                            ...
                            { icon: HardHat, label: 'Phase', value: (prop.progress?.[0]?.label || prop.progress?.[0]?.status_text) || 'In Progress' },
                          ...
                          {prop.progress?.[0] && (
                            <div>
                              <div className="h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                                <motion.div
                                  initial={{ width: 0 }}
                                  animate={{ width: `${(prop.progress[0].percentage ?? prop.progress[0].percent) || 0}%` }}
                                  transition={{ duration: 0.8 }}
                                  className="bg-brand-blue h-full"
                                />
                              </div>
                              <p className="text-[9px] font-black uppercase tracking-widest opacity-40 mt-1">{(prop.progress[0].percentage ?? prop.progress[0].percent)}% complete</p>
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  <table className="w-full min-w-[700px] border-separate border-spacing-x-4">
                    ...
                    <tbody className="text-center font-bold">
                      {[
                        ...
                        { icon: HardHat, label: 'Phase', getValue: (p: SupabaseProperty) => (p.progress?.[0]?.label || p.progress?.[0]?.status_text) || 'In Progress' },
