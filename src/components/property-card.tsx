...
export function PropertyCard({ property, onViewPdf, onViewMap }: { property: SupabaseProperty, onViewPdf: (id: string, title: string) => void, onViewMap: (p: SupabaseProperty) => void }) {
  ...
  let financingBadge: string | null = null;
  if (property.financing_available === true) {
    const down = property.financing_down_payment_pct ?? 20;
    financingBadge = `${down}% Bank Financing`;
  } else if (property.financing_available === false) {
    financingBadge = null;
  } else if (property.loan_percentage && property.loan_percentage > 0) {
    financingBadge = `${property.loan_percentage}% Bank Financing`;
  } else {
    financingBadge = getLoanPercentage(property.developer, property.name);
  }
                  ...
                  <AnimatePresence mode="wait">
                    {activeTab === 'units' ? (
                      ...
                      >
                        ...
                        {property.financing_available === true && (
                          <div className="mt-4 p-4 bg-brand-blue/5 border border-brand-blue/10 rounded-xl space-y-3">
                            <p className="text-[9px] font-black uppercase tracking-widest text-brand-blue flex items-center gap-1.5"><CheckCircle size={10} /> Financing Available</p>
                            <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-[9px] font-bold">
                              {property.financing_types && property.financing_types.length > 0 && (
                                <><div className="opacity-60">Options</div><div className="text-right">{property.financing_types.join(", ")}</div></>
                              )}
                              {property.financing_down_payment_pct !== undefined && property.financing_down_payment_pct !== null && (
                                <><div className="opacity-60">Min Down Payment</div><div className="text-right">{property.financing_down_payment_pct}%</div></>
                              )}
                              {property.financing_duration_months && (
                                <><div className="opacity-60">Duration</div><div className="text-right">{property.financing_duration_months} Months</div></>
                              )}
                              {property.financing_interest_rate !== undefined && property.financing_interest_rate !== null && (
                                <><div className="opacity-60">Interest Rate</div><div className="text-right">{property.financing_interest_rate}% / yr</div></>
                              )}
                              {property.financing_provider && (
                                <><div className="opacity-60">Provider</div><div className="text-right">{property.financing_provider}</div></>
                              )}
                            </div>
                            {property.financing_notes && (
                              <div className="pt-2 border-t border-brand-blue/10">
                                <p className="text-[8px] font-black uppercase opacity-40 mb-1">Eligibility Notes</p>
                                <p className="text-[9px] font-bold opacity-60 leading-relaxed italic">&quot;{property.financing_notes}&quot;</p>
                              </div>
                            )}
                          </div>
                        )}
